<template>
  ...
  <div class="sidebar-box">
    ...
    <ul class="mx-auto">
      <li class="pb-3">
        <span class="text-sm font-medium my-face">{{ t('settings.running-time') }}:</span>
        <span class="text-sm font-medium text-right float-right my-face" v-if="websiteCreateTime != ''">
          {{ websiteCreateTime }}
        </span>
        <ob-skeleton v-else class="float-right" tag="span" width="136px" height="16px" />
      </li>
      <li class="pb-2">
        <span class="text-sm font-medium my-face">{{ t('settings.view-count') }}:</span>
        <span class="text-sm font-medium text-right float-right my-face" v-if="viewCount">{{ viewCount }}</span>
